...
public:
	...
	// 描画準備
	//================================================
	// 陰影をつけるオブジェクト等を描画する前後に行う(スキンメッシュ対応)
	void BeginLit_SkinMesh();
	void EndLit_SkinMesh();
	...
	// 最も初めに行う、光を遮るオブジェクトを描画する前後に行う(スキンメッシュ対応)
	void BeginGenerateDepthMapFromLight_SkinMesh();
	void EndGenerateDepthMapFromLight_SkinMesh();
...
private:
	...
	ID3D11VertexShader* m_VS_Skin = nullptr;				// スキンメッシュ(スキンメッシュ対応)

	ID3D11VertexShader* m_VS_GenDepthFromLight = nullptr;	// 光からの深度
	ID3D11VertexShader* m_VS_GenDepthFromLight_Skin = nullptr;	// 光からの深度(スキンメッシュ対応)

	// 頂点入力レイアウト
	ID3D11InputLayout* m_inputLayout = nullptr;
	ID3D11InputLayout* m_inputLayout_Skin = nullptr;	// スキンメッシュ対応
